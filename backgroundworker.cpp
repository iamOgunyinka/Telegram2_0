...
BackgroundWorker::BackgroundWorker( std::shared_ptr<td::Client>& client, CustomRequestHandlerMap&
                                    handlers_, bool& running, QObject* parent_window,
                                    unsigned long long & auth_id,
                                    QObject* parent ): QObject{ parent }, client_{ client },
    request_handlers_{ handlers_ }, running_{ running }, authentication_query_id_{ auth_id },
...
void BackgroundWorker::ProcessResponse( ResponsePtr r )
{
    if( !r->object ) return;
    if( r->id == 0 ) ProcessUpdate( std::move( r->object ) );
    else {
        auto it = request_handlers_.find( r->id );
        if (it != request_handlers_.end()) {
            it->second( std::move( r->object ) );
        }
    }
}
...
{
    td_api::downcast_call( *ptr, overloaded(
